...
pub fn radius_filter(lat: f64, lon: f64, radius_m: f64) -> String {
    // Use a degree-based bbox for R-tree acceleration, then precise haversine filter
...
pub fn query(
    ...
    // Determine table and select clause
    ...
    let select = match select_cols {
        ...
        None => {
            if spatial_where.is_some() {
                // Select all columns except geom from raw_data
